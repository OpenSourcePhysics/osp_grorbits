...
public class OrbitRainM extends Orbit{
  ...
  public double getVm(double r){
    ...
    c=Math.sqrt(2*r);
    m=1;
    if(b*b-c*c<0) return 0; //r between inner and outer horizon...Vm is complex; we replace it by -Infinity
    else return (a*c*c*pPhi + Math.sqrt(b*b-c*c)*Math.sqrt((r*r*b*b+a*a*c*c)*(r*r*m*m)+r*r*r*r*pPhi*pPhi))/(r*r*b*b+a*a*c*c);
  }
...
class InitialConditionsRainM extends InitialConditionsM{
  ...
  public void adjustEmLmSign(){
    ...
    if(Math.cos(theta0)>0) this.sign=1;
    else this.sign=-1;
